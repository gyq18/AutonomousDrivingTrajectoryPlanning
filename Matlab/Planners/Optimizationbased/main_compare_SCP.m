%  This is the template for comparing different SCP methods in unstructured environments. 
% The randomly generated obstacles are oriented in different directions.
...
global params_
collision_choice = 2;
% [trajectory,isfeasible] = PlanSCPTrajectory(trajectory,collision_choice);

%% L1
t_s0=tic;
[trajectory_s,isfeasible] = PlanL1SCPTrajectory(trajectory,collision_choice);
t_s = toc(t_s0);

%% L2
t_l0=tic;
[trajectory_l,isfeasible] = PlanL2SCPTrajectory(trajectory,collision_choice);
t_l = toc(t_l0);

%% show comparison results
path_runtime=['Runtime: L1: ',num2str(t_s),' L2: ',num2str(t_l)];
disp(path_runtime);
...
hs=plot(trajectory_s.x,trajectory_s.y,'LineWidth',1.5); % 'Color',[1 201/255 14/255]
hold on;
hl=plot(trajectory_l.x,trajectory_l.y,'LineWidth',1.5); %[0 162/255 232/255]
hold on;
...
% VisualizeDynamicResults(trajectory_s);
% VisualizeDynamicResults(trajectory_l);
