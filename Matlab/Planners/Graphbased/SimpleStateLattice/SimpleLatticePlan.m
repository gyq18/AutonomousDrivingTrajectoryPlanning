function [x, y, theta, path_length, is_complete] = SimpleLatticePlan(loc2pos_scale, lut_file, interpolation_file)
    %SimpleLatticePlan Plan the path using Lattice planning method
    ...
    is_complete = 1;
    fprintf("Astar+interpolation time consume: %d \n", cputime-tStart);
...
function cost = hlut(lut, graph, start_pos, start_direction, end_pos, end_direction)
    ...
    if (delta_x < edge_length) && (delta_y < edge_length)
        ...
        return;
    % else calulate the overflow_factor to estimate the result.
    elseif (delta_x < edge_length) && (delta_y >= edge_length)
        overflow_factor = (delta_y + 1) / edge_length;
        delta_x = round(delta_x * edge_length / (delta_y + 1));
        delta_y = edge_length;
    elseif (delta_x >= edge_length) && (delta_y < edge_length)
        overflow_factor = (delta_x + 1) / edge_length;
        delta_y = round(delta_y * edge_length / (delta_x + 1));
        delta_x = edge_length;
    else
        if (delta_x > delta_y)
            overflow_factor = (delta_x + 1) / (delta_y + 1);
        else
            overflow_factor = (delta_y + 1) / (delta_x + 1);
        end
        temp_delta_x = min(round(edge_length * (delta_x + 1) / (delta_y + 1)), edge_length - 1);
        temp_delta_y = min(round(edge_length * (delta_y + 1) / (delta_x + 1)), edge_length - 1);
        delta_x = temp_delta_x;
        delta_y = temp_delta_y;
    end
    
    cost = lut.lookup(start_direction, [delta_x, delta_y], end_direction) * overflow_factor;
