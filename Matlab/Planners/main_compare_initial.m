%  This is the template for MATLAB Source Codes of tesing different initial guesses
%  in static unstructured environments. The randomly generated obstacles are
...
% %% kinematicRRT*
% t_k0=tic;
% [x, y, theta, path_length_k, completnessflag] = PlankinematicRRTStarPath();
% trajectory_k = PlanSpeedForStaticScenario(x, y, theta, path_length_k);
% t_k = toc(t_k0);
...
trajectory_si = PlanSpeedForStaticScenario(x, y, theta, path_length_si);
t_si = toc(t_si0);

%% ImprovedLattice
t_i0=tic;
[x, y, theta, path_length_i, completnessflag] = PlanImprovedLatticePath(1, "look_up_table_121628.txt", "insert_points.txt");
...
for i = 2:length(trajectory_s.theta)
    path_length_oa = path_length_oa + hypot(y(i) - y(i - 1), x(i) - x(i - 1));
end
ot_a = toc(ot_a0); 
at_a = toc(ot_a0)+t_a; 
...
%% Hybrid A*+SLIFS
ot_h0=tic;
[trajectory_s, isFeasible] = PlanL1SCPTrajectory(trajectory_h,collision_choice);
path_length_oh = 0;
for i = 2:length(trajectory_s.theta)
    path_length_oh = path_length_oh + hypot(y(i) - y(i - 1), x(i) - x(i - 1));
end
ot_h = toc(ot_h0); 
at_h = toc(ot_h0)+t_h; 
...
%% CL RRT*+SLIFS
ot_cl0=tic;
[trajectory_s, isFeasible] = PlanL1SCPTrajectory(trajectory_cl,collision_choice);
path_length_s = 0;
for i = 2:length(trajectory_s.theta)
    path_length_s = path_length_s + hypot(y(i) - y(i - 1), x(i) - x(i - 1));
end
ot_cl = toc(ot_cl0); 
at_cl = toc(ot_cl0)+t_cl; 

%% SimpleLattice+SLIFS
ot_si0=tic;
[trajectory_s, isFeasible] = PlanL1SCPTrajectory(trajectory_si,collision_choice);
path_length_s = 0;
for i = 2:length(trajectory_s.theta)
    path_length_s = path_length_s + hypot(y(i) - y(i - 1), x(i) - x(i - 1));
end
ot_si = toc(ot_si0); 
at_si = toc(t_si0)+t_si; 

%% ImprovedLattice+SLIFS
ot_i0=tic;
[trajectory_s, isFeasible] = PlanL1SCPTrajectory(trajectory_i,collision_choice);
path_length_s = 0;
for i = 2:length(trajectory_s.theta)
    path_length_s = path_length_s + hypot(y(i) - y(i - 1), x(i) - x(i - 1));
end
...
path_runtime=['Initial runtime: A*', num2str(t_a),' Hybrid A*: ',num2str(t_h),' CLRRT*: ',num2str(t_cl),' Simple Lattice: ',num2str(t_si),' Improved Lattice: ',num2str(t_i),' Bezier: ',num2str(t_b)];
disp(path_runtime);
path_length=['Initial length: A*', num2str(path_length_a),'Hybrid A*: ',num2str(path_length_h),' CLRRT*: ',num2str(path_length_cl),' Simple Lattice: ',num2str(path_length_si),' Improved Lattice: ',num2str(path_length_i),' Bezier: ',num2str(path_length_b)];
disp(path_length);
path_optime=['Optimization runtime: A*', num2str(ot_a),'Hybrid A*: ',num2str(ot_h),' CLRRT*: ',num2str(ot_cl),' Simple Lattice: ',num2str(ot_si),' Improved Lattice: ',num2str(ot_i),' Bezier: ',num2str(ot_b)];
disp(path_optime);
path_alltime=['Total runtime: A*', num2str(at_a),'Hybrid A*: ',num2str(at_h),' CLRRT*: ',num2str(at_cl),' Simple Lattice: ',num2str(at_si),' Improved Lattice: ',num2str(at_i),' Bezier: ',num2str(at_b)];
disp(path_alltime);
...
ha=plot(trajectory_a.x,trajectory_a.y,'LineWidth',1.5);
hold on;
...
hold on;
legend([ha hh hcl hsi hi hb],{'A*', 'Hybrid A*','CLRRT*','Simple Lattice','Improved Lattice','Bezier'});
