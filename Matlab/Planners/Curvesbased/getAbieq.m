function [Aieq, bieq] = getAbieq(n_order, corridor_range, ts)
    global vehicle_kinematics_
    v_max = vehicle_kinematics_.vehicle_v_max;
    a_max = vehicle_kinematics_.vehicle_a_max;
    j_max = vehicle_kinematics_.vehicle_jerk_max;
    v_min = vehicle_kinematics_.vehicle_v_min;
    a_min = vehicle_kinematics_.vehicle_a_min;
    j_min = vehicle_kinematics_.vehicle_jerk_min;
    n_seg = size(ts,1);
...
%    bieq_v = ones(n_eq,1)* v_max;
    bieq_v_max = ones(n_eq/2,1)* v_max;
    bieq_v_min = ones(n_eq/2,1)* v_min;
...
%     bieq_a = ones(n_eq,1)*a_max;
    bieq_a_max = ones(n_eq/2,1)*a_max;
    bieq_a_min = ones(n_eq/2,1)*a_min;
...
%    bieq_j = ones(n_eq,1)*j_max;
    bieq_j_max = ones(n_eq/2,1)*j_max;
    bieq_j_min = ones(n_eq/2,1)*j_min;
    bieq_j = [bieq_j_max;-bieq_j_min];
