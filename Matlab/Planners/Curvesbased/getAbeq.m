function [Aeq, beq] = getAbeq(n_order, ts,axis)
    ...
    Aeq_start = zeros(3, n_all_poly); % Ascending order
    ...
    if(axis==1)
        beq_start =  [vehicle_TPBV_.x0,0,0]';
    else
        beq_start =  [vehicle_TPBV_.y0,0,0]';
    end
    ...
    Aeq_end = zeros(3, n_all_poly); % Descending order
    ...
    if(axis==1)
        beq_end = [vehicle_TPBV_.xtf,0,0]';
    else
        beq_end = [vehicle_TPBV_.ytf,0,0]';
    end
    %#####################################################
