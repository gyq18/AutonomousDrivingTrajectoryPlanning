%  This is the template for MATLAB Source Codes of test the bezier-based planner. The randomly generated obstacles are
...
vehicle_TPBV_.xtf=52; vehicle_TPBV_.ytf=52;vehicle_TPBV_.thetatf=0;vehicle_TPBV_.vtf=0;vehicle_TPBV_.phitf=0;vehicle_TPBV_.a=0;vehicle_TPBV_.omegatf=0;
...
[x, y, theta, path_length, completeness_flag,box_list] = PlanBezierPath(); %including: CreateDilatedCostmap(),ResampleConfig(x, y, theta);
% [x, y, theta, path_length, completeness_flag] = PlanPRMPath();
...
function VisualizeStaticResults(trajectory,box_list)
...
if draw_box
    for i= 1:size(box_list,1)
        px = [box_list{i}(1,1),box_list{i}(2,1),box_list{i}(4,1),box_list{i}(3,1),box_list{i}(1,1)];
        py = [box_list{i}(1,2),box_list{i}(2,2),box_list{i}(4,2),box_list{i}(3,2),box_list{i}(1,2)];
        plot(px, py,'Color',[180/255 20/255 220/255],'LineWidth',1); 
        hold on;
    end
end
