...
vehicle_TPBV_.xtf=54; vehicle_TPBV_.ytf=34;vehicle_TPBV_.thetatf=pi;vehicle_TPBV_.vtf=-1;vehicle_TPBV_.phitf=0;vehicle_TPBV_.a=0;vehicle_TPBV_.omegatf=0;
...
trajectory_atar0 = PlanSpeedForStaticScenarios(x, y, theta, path_length);
collision_choice = 2;
[trajectory_opt0,isfeasible] = PlanL1SCPTrajectory(trajectory_atar0,collision_choice);

VisualizeStaticResults(trajectory_atar0);
VisualizeStaticResults(trajectory_opt0);
trajectory_length_astar=0;
for kk=1:(length(trajectory_atar0.x)-1)
    trajectory_length_astar = trajectory_length_astar + hypot(trajectory_atar0.x(kk+1)-trajectory_atar0.x(kk),trajectory_atar0.y(kk+1)-trajectory_atar0.y(kk));
end    
nstep_astar = round(trajectory_length_astar);
trajectory_length_opt=0;
for kk=1:(length(trajectory_opt0.x)-1)
    trajectory_length_opt = trajectory_length_opt + hypot(trajectory_opt0.x(kk+1)-trajectory_opt0.x(kk),trajectory_opt0.y(kk+1)-trajectory_opt0.y(kk));
end    
nstep_opt = round(trajectory_length_opt);

if nstep_astar<= 90 && nstep_opt<= 90 

delta_theta_astar = zeros(nstep_astar-1,1);
delta_theta_opt = zeros(nstep_opt-1,1);
dist_astar = zeros(nstep_astar-1,1);
dist_opt = zeros(nstep_opt-1,1);
trajectory_atar1.x(1)=4; trajectory_atar1.y(1)=4; trajectory_atar1.theta(1)=0;
trajectory_opt1.x(1)=4; trajectory_opt1.y(1)=4; trajectory_opt1.theta(1)=0;

%% 使两点之间的距离尽量平均
% theta_astar = zeros(nstep_astar-1,1);
% theta_opt = zeros(nstep_opt-1,1);
theta_astar = ResampleThetaWithEqualDistance(trajectory_atar0.x, trajectory_atar0.y,trajectory_atar0.theta,trajectory_length_astar);
theta_opt = ResampleThetaWithEqualDistance(trajectory_opt0.x, trajectory_opt0.y,trajectory_opt0.theta,trajectory_length_opt);


for ii = 1 :(nstep_astar-1)
    %% 两点之间距离的平均化
    %% Astar theta离散归一化的程序
    delta_theta_astar(ii) = theta_astar(ii+1)-theta_astar(ii);
    delta_theta_astar(ii) = round(delta_theta_astar(ii),2);
    trajectory_atar1.theta(ii+1) = trajectory_atar1.theta(ii) + delta_theta_astar(ii);
    trajectory_atar1.x(ii+1) = trajectory_atar1.x(ii)+cos(trajectory_atar1.theta(ii));
    trajectory_atar1.y(ii+1) = trajectory_atar1.y(ii)+sin(trajectory_atar1.theta(ii));
end

for ii = 1 :(nstep_opt-1)
    %% 两点之间距离的平均化
    %% Optimizaiton theta离散归一化的程序
    delta_theta_opt(ii) = theta_opt(ii+1)-theta_opt(ii);
    delta_theta_opt(ii) = round(delta_theta_opt(ii),2);
    trajectory_opt1.theta(ii+1) = trajectory_opt1.theta(ii) + delta_theta_opt(ii);
    trajectory_opt1.x(ii+1) = trajectory_opt1.x(ii)+cos(trajectory_opt1.theta(ii));
    trajectory_opt1.y(ii+1) = trajectory_opt1.y(ii)+sin(trajectory_opt1.theta(ii));
end

for ii = 1 :(nstep_astar-1) 
   %% Astar theta离散归一化长度
    dist_astar(ii) = hypot(trajectory_atar1.x(ii+1)-trajectory_atar1.x(ii), trajectory_atar1.y(ii+1)-trajectory_atar1.y(ii));
end

for ii = 1 :(nstep_opt-1) 
    %% Optimizaiton theta离散归一化的程序
    dist_opt(ii) = hypot(trajectory_opt1.x(ii+1)-trajectory_opt1.x(ii), trajectory_opt1.y(ii+1)-trajectory_opt1.y(ii));
end

% for ii = 1 :(nstep-1)  
%    %% Astar theta离散归一化的程序
%     dist_astar(ii) = hypot(trajectory_atar0.x(ii+1)-trajectory_atar0.x(ii), trajectory_atar0.y(ii+1)-trajectory_atar0.y(ii));
%     delta_theta_astar(ii) = trajectory_atar0.theta(ii+1)-trajectory_atar0.theta(ii);
%     delta_theta_astar(ii) = round(delta_theta_astar(ii),2);
%     trajectory_atar1.theta(ii+1) = trajectory_atar1.theta(ii) + delta_theta_astar(ii);
%     trajectory_atar1.x(ii+1) = trajectory_atar1.x(ii)+dist_astar(ii)*cos(trajectory_atar1.theta(ii));
%     trajectory_atar1.y(ii+1) = trajectory_atar1.y(ii)+dist_astar(ii)*sin(trajectory_atar1.theta(ii));
%     %% Optimizaiton theta离散归一化的程序
%     dist_opt(ii) = hypot(trajectory_opt0.x(ii+1)-trajectory_opt0.x(ii), trajectory_opt0.y(ii+1)-trajectory_opt0.y(ii));
%     delta_theta_opt(ii) = trajectory_opt0.theta(ii+1)-trajectory_opt0.theta(ii);
%     delta_theta_opt(ii) = round(delta_theta_opt(ii),2);
%     trajectory_opt1.theta(ii+1) = trajectory_opt1.theta(ii) + delta_theta_opt(ii);
%     trajectory_opt1.x(ii+1) = trajectory_opt1.x(ii)+dist_opt(ii)*cos(trajectory_opt1.theta(ii));
%     trajectory_opt1.y(ii+1) = trajectory_opt1.y(ii)+dist_opt(ii)*sin(trajectory_opt1.theta(ii));
% end
VisualizeStaticResults(trajectory_atar1);
VisualizeStaticResults(trajectory_opt1);
...
function theta = ResampleThetaWithEqualDistance(x, y,theta,trajlength)
nstep_new = round(trajlength);
theta_extended = [];
for ii = 1 :(length(x) - 1) %59
...
index = round(linspace(1, length(theta_extended), nstep_new));
