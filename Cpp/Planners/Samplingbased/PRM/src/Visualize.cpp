...
void VisualizeStaticResults(struct Trajectory trajectory, vector<Bezier::bounding_box> box_list) {
    ...
    vector<math::Vec2d> center_list;
    for (int i = 0; i < box_list.size(); i++) {
        center_list.push_back(box_list[i].center);
    }
    vector<Point> P_center = axis.plist_in_canvas(center_list, canvas);
    for (int i = 0; i < box_list.size(); ++i) {
        // draw corridor
        vector<math::Vec2d> P;
        P.push_back(box_list[i].P1);
        P.push_back(box_list[i].P2);
        P.push_back(box_list[i].P4);
        P.push_back(box_list[i].P3);
        vector<Point> V_p = axis.plist_in_canvas(P, canvas);
        polylines(canvas, V_p, true, Scalar(100, 217, 153), 1, 8, 0);

        circle(canvas, P_center[i], 2, Scalar(0, 0, 255), -1);
    }
    // draw corridor
...
void VisualizeDynamicResults(struct Trajectory trajectory) {
    int nstep = trajectory.x.size();
    double xmax = *max_element(trajectory.x.begin(), trajectory.x.end());//���ֵ
    double xmin = *min_element(trajectory.x.begin(), trajectory.x.end());//��Сֵ
    double ymax = *max_element(trajectory.y.begin(), trajectory.y.end());//���ֵ
    double ymin = *min_element(trajectory.y.begin(), trajectory.y.end());//��Сֵ
    cv_axis2d axis(xmin - 5, xmax + 5, ymin - 5, ymax + 5);
    Mat canvas = Mat::ones(Size(512, 512 * (axis.y_h - axis.y_l) / (axis.x_h - axis.x_l)), CV_8UC3);
