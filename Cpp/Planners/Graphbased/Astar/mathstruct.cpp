...
vector < vector<math::Vec2d>> GenerateStaticObstacles_unstructured() {
    ...
    while (count < Nobs) {
        ...
        math::Vec2d tempoint;
        tempoint.set_x(x - margin_obs_);tempoint.set_x(y + margin_obs_);
        temp_obj_margin.push_back(tempoint);
        tempoint.set_x(xru + margin_obs_);tempoint.set_x(yru + margin_obs_);
        temp_obj_margin.push_back(tempoint);
        tempoint.set_x(xrd + margin_obs_);tempoint.set_x(yrd - margin_obs_);
        temp_obj_margin.push_back(tempoint);
        tempoint.set_x(xld - margin_obs_);tempoint.set_x(yld - margin_obs_);
        temp_obj_margin.push_back(tempoint);
        tempoint.set_x(x - margin_obs_);tempoint.set_x(y + margin_obs_);
        temp_obj_margin.push_back(tempoint);
