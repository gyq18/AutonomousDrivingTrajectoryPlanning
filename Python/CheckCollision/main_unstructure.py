...
def GenerateStaticObstacles_unstructured():
    ...
    #
    V_initial = CreateVehiclePolygon(
        vehicle_TPBV_.x0, vehicle_TPBV_.y0, vehicle_TPBV_.theta0)
    V_terminal = CreateVehiclePolygon(
        vehicle_TPBV_.xtf, vehicle_TPBV_.ytf, vehicle_TPBV_.thetatf)
    count = 0
    obj = np.array([])
    ...
    while count < Nobs:
        ...
        xru = x + L * np.cos(theta)
        yru = y + L * np.sin(theta)
        xrd = xru + W * np.sin(theta)
        yrd = yru - W * np.cos(theta)
        xld = x + W * np.sin(theta)
        yld = y - W * np.cos(theta)
        ...
        xv = np.array([x - margin_obs_, xru + margin_obs_, xrd +
                      margin_obs_, xld - margin_obs_, x - margin_obs_])
        yv = np.array([y + margin_obs_, yru + margin_obs_, yrd -
                      margin_obs_, yld - margin_obs_, y + margin_obs_])
        temp_obj_margin = np.array([[x - margin_obs_, xru + margin_obs_, xrd + margin_obs_, xld - margin_obs_, x - margin_obs_], [
                                   y + margin_obs_, yru + margin_obs_, yrd - margin_obs_, yld - margin_obs_, y + margin_obs_]])
        ...
        vehicle_state = np.array(
            [V_initial.x[0:4], V_initial.y[0:4], V_terminal.x[0:4], V_terminal.y[0:4]])
        if count > 0:
            flag1 = checkObj_linev(
                temp_obj_margin[:, 0], temp_obj_margin[:, 1], vehicle_state)
            flag2 = checkObj_linev(
                temp_obj_margin[:, 3], temp_obj_margin[:, 2], vehicle_state)
            flag3 = checkObj_linev(
                temp_obj_margin[:, 0], temp_obj_margin[:, 2], vehicle_state)
            flag4 = checkObj_linev(
                temp_obj_margin[:, 1], temp_obj_margin[:, 3], vehicle_state)
            if flag1 + flag2 + flag3 + flag4 > 0:
                label_obj = 1
...
def VisualizeStaticResults(trajectory):
    ...
    plt.plot(trajectory.x, trajectory.y, '.-',
             color=(1, 127 / 255, 39 / 255), markersize=2, lw=1)
    # plt.hold(True)
    # plot vehicle body
    ...
    plt.plot(trajectory.x[0], trajectory.y[0], 'o',
             color=(1, 201 / 255, 14 / 255), lw=1)
    # plt.hold(True)
    plt.plot(trajectory.x[nstep-1], trajectory.y[nstep-1],
             'p', color=(1, 201 / 255, 14 / 255), lw=1)
    # plt.hold(True)
    plt.axis(np.array([planning_scale_.xmin, planning_scale_.xmax,
             planning_scale_.ymin, planning_scale_.ymax]))
    plt.axis('equal')
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    # plt.hold(True)
...
if __name__ == "__main__":
    ...
    from CheckByAABB import CheckByAABB
    from CheckByOBB import CheckByOBB
    from CheckByArea import CheckByArea
    from CheckByCircle import CheckByCircle
    from CheckByLine import CheckByLine
    from CheckByMap import CheckByMap
    ...
    print('CheckByAABB   ', Is_collision_aabb)
    print('time')
    print(time_aabb)
    print('CheckByOBB    ', Is_collision_obb)
    print('time')
    print(time_obb)
    print('CheckByCircle ', Is_collision_circle)
    print('time')
    print(time_circle)
    print('CheckByArea   ', Is_collision_area)
    print('time')
    print(time_area)
    print('CheckByLine   ', Is_collision_line)
    print('time')
    print(time_line)
    print('CheckByMap    ', Is_collision_map)
    print('time')
    print(time_map)
