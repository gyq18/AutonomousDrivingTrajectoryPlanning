...
def kinematiccon(x, nstep, Apdyn, Aqdyn, Athetadyn, Lm):
    ...
    ceq = np.concatenate([np.dot(Apdyn, x[0:2 * nstep]) - np.multiply(Ts * vel[0:nstep - 1], np.cos(theta[0:nstep - 1])),
                         np.dot(Aqdyn, x[0:2 * nstep]) - np.multiply(Ts * vel[0:nstep - 1], np.sin(theta[0:nstep - 1])),
...
def Find_Box_for_all_obj(xr):
    ...
    def IsPointValidInDilatedMap(x, y):
        ...
        ind_x = math.floor((x - planning_scale_.xmin) / hybrid_astar_.resolution_x) + 1
        ind_y = math.floor((y - planning_scale_.ymin) / hybrid_astar_.resolution_y) + 1
    ...
    def IsCurrentExpansionValid(xc, yc, test, lb, ind):
        ...
        ind_x = np.floor((xx - planning_scale_.xmin) / hybrid_astar_.resolution_x) + 1
        ind_y = np.floor((yy - planning_scale_.ymin) / hybrid_astar_.resolution_y) + 1
...
def PlanOptimizationL2Trajectory(initialguess, collision_choice):
    # 用L2范数处理非线性运动学等式方程，车辆用一个圆覆盖
    ...
    # 离散的代数状态方程
    Av1 = np.zeros((nstep - 1, num_dv))
    Av2 = np.zeros((nstep - 1, num_dv))
    Aphi1 = np.zeros((nstep - 1, num_dv))
    Aphi2 = np.zeros((nstep - 1, num_dv))

    # 新的
    Obj1 = np.zeros((nstep - 1, num_dv))
    Obj2 = np.zeros((nstep - 1, num_dv))
    ...
    # 边界条件约束
    lb = np.zeros((num_dv, 1))
    ub = np.zeros((num_dv, 1))
    # 轨迹横纵坐标的范围限制
    ...
    for k in range(maxiter):
        ...
        # 避障约束
        A = np.zeros((num_collision_max, num_dv))
        ...
        for i in range(1, nstep + 1):
            ...
            if num_tempA > 0:
                if(counter_collision - 1 + num_tempA > A.shape[0]):
                        A = np.concatenate([A, np.zeros((counter_collision - 1 + num_tempA - A.shape[0], num_dv))],axis = 0)
                        b = np.concatenate([b, np.zeros((counter_collision - 1 + num_tempA - b.shape[0], 1))],axis = 0)        
                A[counter_collision - 1: counter_collision - 1 + num_tempA, 2 * (i - 1): 2 * i] = tempA * 1
                if collision_choice == 1:
                    b[counter_collision - 1: counter_collision - 1 + num_tempA] = tempb * 1
                elif collision_choice == 2:
                    b[counter_collision - 1: counter_collision - 1 + num_tempA] = tempb - o_margin
        ...
        Aineq = np.zeros((num_constraint, num_dv))
        ...
        Aineq[num_collision + 4 * nstep - 4: num_collision +
              4 * nstep - 4 + num_dv] = Tr * 1
        Aineq[num_collision + 4 * nstep - 4 +
              num_dv: num_collision + 4 * nstep - 4 + 2 * num_dv] = -Tr * 1
        ...
        for inner_index in range(1, inner_maxiter):
            # x0+radius
            bineq[num_collision + 4 * nstep - 4:
                  num_collision + 4 * nstep - 4 + num_dv] = x0 + radius
            # -x0+radius
            bineq[num_collision + 4 * nstep - 4 + num_dv:
                  num_collision + 4 * nstep - 4 + 2 * num_dv] = -x0 + radius

            cost_counter = cost_counter + 1

            # 在未计算前就统计约束违反情况
            ceq1 = np.dot(Aeq0, x0) - beq0
            theta = x0[2 * nstep: 3 * nstep] * 1
            vel = x0[3 * nstep: 4 * nstep] * 1
            phi = x0[4 * nstep: 5 * nstep] * 1
            Ts = x0[5 * nstep][0] / (nstep - 1)

            ceq2 = np.concatenate([np.dot(Apdyn, x0[:2 * nstep]) - np.multiply(np.dot(Ts, vel[:nstep - 1]), np.cos(theta[:nstep - 1])),
                                   np.dot(Aqdyn, x0[:2 * nstep]) - np.multiply(
            ...
            # 不等式
            cineq1 = np.dot(Aineq[: num_dv], x0) - bineq[: num_dv]
            cineq2 = lb - x0
            cineq3 = x0 - ub
            ...
            # 求解底层优化问题
            H2 = H + 2 * lambda1 * np.dot(delta_Jac.T, delta_Jac) ##改变目标函数 注意由于原先是0.5*x'*H*x+f*x,所以H2中lambda之前要乘以2
            f2 = f - 2 * lambda1 * np.dot(delta_Jac.T, delta_b)
            x, exitflag = quadprog(H2, f2, Aineq, bineq, Aeq0, beq0, lb, ub)
            if (not (exitflag == 'optimal')) or (sum(np.isnan(x)) > 0) or (x.any() == False):
                isFeasible = 0
                trajectory.x = x_last[: dim * nstep: 2] * 1
                trajectory.y = x_last[1: dim * nstep: 2] * 1
                trajectory.theta = x_last[2 * nstep: 3 * nstep] * 1
                trajectory.v = x_last[3 * nstep: 4 * nstep] * 1
                trajectory.phi = x_last[4 * nstep: 5 * nstep] * 1
            ...
            delta_J = J_x0 - J_x
            aux = np.dot(delta_Jac, x) - delta_b
    ...
    if isFeasible:
        ...
        if sum(lb - x) <= 0 and sum(x - ub) <= 0 and sum(np.dot(Aineq, x) - bineq) <= 0 and sum(np.fabs(np.dot(Aeq0, x) - beq0)) <= 0.1:
            isFeasible = 1
        else:
            isFeasible = 0
    ...
    # 这段代码找到的可行解似乎只是原轨迹
    if isFeasible == 0 or x.any() == False:
        print(isFeasible)
        fun1 = lambda x1, H_x1, f1, nstep1, Apdyn1, Aqdyn1, Athetadyn1, Lm1: 1/2 * np.dot(np.dot(x1.T, H_x1), x1) + np.dot(f1.T, x1)
        
        tmp=[[lb[i],ub[i]] for i in range(len(lb))]
        bounds = tuple(tuple(i) for i in tmp)
        cons = ({'type':'ineq','fun':lambda x1, H_x1, f1, nstep1, Apdyn1, Aqdyn1, Athetadyn1, Lm1: kinematiccon(x1, nstep1, Apdyn1, Aqdyn1, Athetadyn1, Lm1),'args':(H_x, f, nstep, Apdyn, Aqdyn, Athetadyn, Lm)})

        Jac1 = ({'fun':lambda x1, H_x1, f1, nstep1, Apdyn1, Aqdyn1, Athetadyn1, Lm1: ComputeJac(x1, Lm1), 'args':(H_x, f, nstep, Apdyn, Aqdyn, Athetadyn, Lm)})
        
        res = minimize(fun1, x0, args = (H_x, f, nstep, Apdyn, Aqdyn, Athetadyn, Lm), method='SLSQP',jac = Jac1, bounds = bounds, constraints = cons)#, options={'disp': False}
        x = res.x * 1
        if (x.any() == False) or (res.message != 'Optimization terminated successfully.'):
            isFeasible = 0
            x = np.concatenate([refpath_vec, theta0, v0, phi0, tf0], axis=0)
