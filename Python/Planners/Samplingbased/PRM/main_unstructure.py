...
def CreateVehiclePolygon(x, y, theta):
    ...
    AX = x + (
            vehicle_geometrics_.vehicle_front_hang + vehicle_geometrics_.vehicle_wheelbase) * cos_theta - vehicle_half_width * sin_theta
    BX = x + (
            vehicle_geometrics_.vehicle_front_hang + vehicle_geometrics_.vehicle_wheelbase) * cos_theta + vehicle_half_width * sin_theta
    ...
    AY = y + (
            vehicle_geometrics_.vehicle_front_hang + vehicle_geometrics_.vehicle_wheelbase) * sin_theta + vehicle_half_width * cos_theta
    BY = y + (
            vehicle_geometrics_.vehicle_front_hang + vehicle_geometrics_.vehicle_wheelbase) * sin_theta - vehicle_half_width * cos_theta
...
def GenerateStaticObstacles_unstructured():
    ...
    while count < Nobs:
        ...
        temp_obj_margin = np.array(
            [[x - margin_obs_, xru + margin_obs_, xrd + margin_obs_, xld - margin_obs_, x - margin_obs_],
             [y + margin_obs_, yru + margin_obs_, yrd - margin_obs_, yld - margin_obs_, y + margin_obs_]])
        if inpolygon(vehicle_TPBV_.x0, vehicle_TPBV_.y0, xv, yv) > 0 or inpolygon(vehicle_TPBV_.xtf, vehicle_TPBV_.ytf,
                                                                                  xv, yv) > 0:
            continue
...
def VisualizeStaticResults(trajectory, simplify_list=None, box_list=None, pt_list=None):
    ...
    if box_list:
        ...
        for i in range(n_box):
            x_list = [box_list[i].P1[0], box_list[i].P2[0], box_list[i].P4[0], box_list[i].P3[0], box_list[i].P1[0]]
            y_list = [box_list[i].P1[1], box_list[i].P2[1], box_list[i].P4[1], box_list[i].P3[1], box_list[i].P1[1]]
            plt.plot(x_list, y_list, color=(153 / 255, 30 / 255, 234 / 255), lw=1)

    if simplify_list:
        if simplify_list == box_list:
            pass
        else:
            n_box = len(simplify_list)
            for i in range(n_box):
                x_list = [simplify_list[i].P1[0], simplify_list[i].P2[0], simplify_list[i].P4[0],
                          simplify_list[i].P3[0], simplify_list[i].P1[0]]
                y_list = [simplify_list[i].P1[1], simplify_list[i].P2[1], simplify_list[i].P4[1],
                          simplify_list[i].P3[1], simplify_list[i].P1[1]]
                plt.plot(x_list, y_list, color=(153 / 255, 16 / 255, 20 / 255), lw=1)

    if pt_list:
        n_pt = len(pt_list)
        for i in range(n_pt):
            plt.plot(pt_list[i][0], pt_list[i][1], 'o', color=(1, 201 / 255, 14 / 255), lw=0.5)
