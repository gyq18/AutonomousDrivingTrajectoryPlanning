...
if __name__ == "__main__":
    from CL_RRTStar import CL_RRTStar
    import time
    ...
    start = [globalvar.vehicle_TPBV_.x0, globalvar.vehicle_TPBV_.y0, globalvar.vehicle_TPBV_.v0, globalvar.vehicle_TPBV_.theta0]
    goal  = [globalvar.vehicle_TPBV_.xtf, globalvar.vehicle_TPBV_.ytf, globalvar.vehicle_TPBV_.vtf, globalvar.vehicle_TPBV_.thetatf]
    clrrt = CL_RRTStar(start, goal, globalvar.obstacles_, expand_dis=15.0, connect_circle_dist=15, sim_timestep=0.5, loc_threshold=1.5, goal_sample_rate=0)
    
    tic = time.perf_counter()
    x, y, theta, path_length, completeness_flag = clrrt.planning()
    toc = time.perf_counter()
    print(toc - tic)
    f_n = [node for node in clrrt.node_list if node.reachable]
    print(len(f_n))
    print(len(clrrt.node_list))
